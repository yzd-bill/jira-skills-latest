...
- **Success** â†’ "Setup verified. Proceeding."
- **Failure** â†’ display error, suggest checking MCP connection or Jira permissions

---

## Step 1 â€” Discover which projects have my open sprint tickets
...
Extract unique project keys + names from the results. Cache the full result set in memory â€” Steps 3-6 reuse it.

**If zero results** â†’ tell user "No open tickets in your current sprint across any project." Stop.

---

## Step 2 â€” Ask user which project to scope to
...
**Shortcut**: if only one project found, skip the question. Just inform the user: "Found N open tickets in `<Project>` â€” proceeding."

---

## Step 3 â€” Filter & present numbered list

Filter the Step 1 results to the chosen project (or keep all if `All projects`). Sort by `status` (Backlog â†’ In Progress â†’ In Review â†’ others) then by `key` ascending. Present as numbered list:
...
## Step 4 â€” Ask which to mark Done
...
Parse:
- `all` â†’ `done_set` = all items, `remaining_set` = []
- `none` â†’ `done_set` = [], `remaining_set` = all items
- comma list (e.g. `1,3,5`) â†’ split, validate each is within the displayed range, error out and re-ask on invalid input

---

## Step 5 â€” For each remaining ticket, ask target status
...
## Step 6 â€” Execute (Done batch first, then transitions batch)

Maintain a `failures` list. Tolerate per-ticket errors â€” log and continue.

### 6a â€” Done batch (per ticket in `done_set`)

1. Read `remainingEstimateSeconds` from the cached Step 1 data.
2. **If `remainingEstimateSeconds > 0`**:
   - Convert to Jira duration string (e.g. `5400` â†’ `"1h 30m"`).
   - Call `addWorklogToJiraIssue` with `timeSpent = "<duration>"`. Jira auto-decrements `Remaining Estimate` to 0.
3. **If `remainingEstimateSeconds == 0`** â†’ skip worklog (already fully logged).
4. **If `originalEstimate is null`** â†’ skip worklog silently (no estimate was ever set; nothing to log to zero).
5. **Walk to Done** â€” many Jira workflows do not allow a direct transition to Done from every status. The skill auto-chains through intermediate states:
   - Call `getTransitionsForJiraIssue`. Find a transition whose target status's `statusCategory.key == "done"`.
   - If found â†’ apply via `transitionJiraIssue`. Done.
   - If not found â†’ look for any forward transition (a transition whose target status category differs from the current one and moves toward completion). Apply it, then re-call `getTransitionsForJiraIssue` and look for a `done` transition again.
...
### 6b â€” Transitions batch (per ticket in `remaining_set`)

- If recorded transition ID is `null` â†’ skip.
- Otherwise call `transitionJiraIssue` with the recorded ID.

---

## Step 7 â€” Verify & wrap up
...
No follow-up offers â€” the skill ends here.
...
- Do not hardcode transition IDs to "Done" â€” workflows vary by project and instance. The path to Done may require multiple hops (e.g. Backlog â†’ Next â†’ In Progress â†’ Review â†’ Done). Always call `getTransitionsForJiraIssue` per ticket.
- Do not confuse `remainingEstimate == 0` (already fully logged) with `originalEstimate is null` (no estimate ever set). Both result in "skip worklog" but they're different states â€” log them differently in any debug output.
- Jira duration format: worklog `timeSpent` must be a string like `30m`, `1h`, `1h 30m`, `1d`. Don't pass raw seconds.
- Re-querying full ticket data between steps wastes API calls â€” cache the Step 1 result and reuse `remainingEstimateSeconds` from it for Step 6a.
- `All projects` is not a JQL filter value â€” just don't apply a `project = ...` clause when the user selects it.
